...
function generateAmortizationSchedule({ principal, annualRate, months, date }) {
  if (!date) {
    date = moment().startOf("day").toDate();
  }
  let loan = calculateLoan({
    principal,
    annualRate,
    months
  });
  ...
  if (!startDate.isValid()) {
    throw new Error("date ไม่ถูกต้อง ต้องเป็นรูปแบบ YYYY-MM-DD");
  }
  let schedule = [];
  ...
  for (let i = 1; i <= Number(months); i++) {
    ...
    let dueDate = startDate.clone().add(i, "months").toDate();
    schedule.push({
      installment: i,
      dueDate,
